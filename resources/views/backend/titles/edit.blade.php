<x-admin-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('admin.backend.titles.update', ['region' => $region->id, 'title' => $title->id])  }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                @if ($errors->any())
                ...
                <div class="-m-2">
                    <div class="p-2 w-4/5 mx-auto">
                        ...
                        <input type="text" id="title_name" name="title_name" value="{{ $title->title_name }}" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-yellow-500 focus:bg-white focus:ring-2 focus:ring-yellow-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                        </div>
                    </div>
                </div>
                ...
                <div class="m-2">
                    <div class="p-2 w-4/5 mx-auto">
                        <div class="relative">
                            ...
                            <input type="number" id="total_point" name="total_point" value="{{ $title->total_point }}" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-yellow-500 focus:bg-white focus:ring-2 focus:ring-yellow-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex-container mb-4">
                    <div class="image-input">
                        ...
                        <div class="image-preview mt-2" id="preview_image">
                            <img src="{{ $title->image }}" alt="No Image" class="image-preview">
                        </div>
                    </div>
                </div>
            ...
            <div class="p-2 w-full flex justify-around m-4">
                <form action="{{ route('admin.admin.backend.titles.destroy', ['region' => $region->id, 'title' => $title->id]) }}" method="POST">
                    @csrf
                    @method('DELETE')
